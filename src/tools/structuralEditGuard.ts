import { structuralReferenceImpact } from '../shared/store/stepColumnHelpers';
import { useStore } from '../shared/store';

/** Ask before a structural edit that currently drops unremappable references. */
export function confirmStructuralReferenceLoss(operation: string): boolean {
  const impact = structuralReferenceImpact(useStore.getState().diagram);
  if (impact.nodeAnchors === 0 && impact.dependencyEdges === 0) return true;
  const references = [
